...
从 0.1.2 当前修订开始，本地文件夹项目中的文件遵循“打开什么文件，就保存回什么文件”：

- 打开 `.mdoc`：读取并保存同一个 `.mdoc` 原文件。
- 打开 `.md`：读取 Markdown 内容，编辑后保存回同一个 `.md` 原文件。
- 打开 `.html` / `.htm` / `.txt` / `.docx`：保存回对应原文件。
...
| `.md` | 支持 | 写回原 `.md` | 支持 | 作为 Markdown 内容导入编辑 |
