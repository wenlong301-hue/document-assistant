...
| 图片预览（lightbox） | 全屏 | **450** | 深色遮罩 `bg-black/60`；非标准白底弹窗；Toast 用 `!z-[460]` |
| 更新 | 520 | 400 | 与分享弹窗同壳 |
...
编辑器选中图片后可打开全屏预览（工具栏「预览」或双击图片）：

- 组件：`src/app/editor/ui/ImagePreviewModal.tsx`
- `z-[450]`，遮罩 `bg-black/60`（比普通弹窗更深，便于看清图片）
- 支持滚轮 / `+` `-` 缩放、拖拽平移、双击切换 100%/200%、`0` 重置
- 「复制图片」写入剪贴板（PNG）；成功/失败用 Toast（`!z-[460]`）
- Esc / 点遮罩 / 关闭按钮退出
- 导出 / 局域网分享的独立 HTML（`buildPreviewHtml`）内嵌同等 lightbox：点击正文图片打开，交互与快捷键一致
